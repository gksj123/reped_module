#-*-Python-*-
# Created by likai at 29 Jun 2020  14:47
...
root['OUTPUTS']['TEQ']=OMFITcollection()
...
#inputs = [(OMFIT['REPED']['OUTPUTS']['SAV']['cfetr_inv.sav'],'cfetr_inv.sav'),(OMFIT['REPED']['INPUTS']['jbootstrap'],'jbootstrap.bas')]


nnn = root['SETTINGS']['PHYSICS']['width']  #width
nnnn = root['SETTINGS']['PHYSICS']['height'] #height
ii = 0
nheight = int(height*100) + 1
for i in range(5):
    tt = np.linspace(nnnn[i],nnnn[i]+height,nheight)

    for at00 in tt:
        varpeddir = str(ii).zfill(6)

        print(at00)
        varpedbas = OMFITascii('varpedbas')
        print (varpedbas.filename)
        open(varpedbas.filename,'w').write(
"""
teq_inv(0,0)
...
weqdsk("t")
quit
"""%(betann, nebarr, zionn, zimpp, nnn[i], nepeak, nesepp, an00, alphan0, alphan1, tesep, at00,
     at11, alphat0, alphat1, zefff, c11, c22, varpeddir, at00))
        #root['OUTPUTS']['varpedbas'] = OMFITascii('varpedbas')

        root['OUTPUTS']['varpedbas'] = varpedbas
        inputs = [(root['OUTPUTS']['SAV']['cfetr_inv.sav'],'cfetr_inv.sav'),(root['INPUTS']['jbootstrap'],'jbootstrap.bas'),(root['OUTPUTS']['varpedbas'],'varpedbas')]
        root['OUTPUTS']['TEQ'][varpeddir]=OMFITtree()
        at00 = round(at00,2)
        ...
        executable = "/project/CFETR/bin/pbsMonitor -jn 1 -cn 1 -jq batch -wt 06:00:00 -exe caltrans cfetr_inv.sav  varpedbas"
        #executable=str(root['SETTINGS']['SETUP']['executable'])+' '+'cfetr_inv.sav '+os.path.split(varpedbas.filename)[1]+' | tee CORSICA_log.txt'
        OMFITx.executable(root,inputs,outputs,executable=executable)




        root['OUTPUTS']['TEQ'][varpeddir]['tfile']=OMFITascii(pppp)
        root['OUTPUTS']['TEQ'][varpeddir]['varpedbas']=OMFITascii(varpedbas)
